...
public class OldObjectSystem : MonoBehaviour {
	...
	void Update () {
		...
		if(Input.GetKeyUp(KeyCode.E)) {
			if(!MainCam.GetComponent<OldPlayer>().PositionLock) {
				gameObject.GetComponent<Rigidbody>().isKinematic = false;
			}
		}
		if(Input.GetKeyDown(KeyCode.Backspace) && IsInventoryOpen && string.IsNullOrEmpty(MainCam.GetComponent<OldPlayer>().WTMode)) {
			Hotbar[SelectorCount].sprite = EmptySlotSprite;
			HotbarItems[SelectorCount] = null;
		}
		if(Input.GetKeyDown(KeyCode.Return) && IsInventoryOpen && string.IsNullOrEmpty(MainCam.GetComponent<OldPlayer>().WTMode)) {
			Hotbar[SelectorCount].sprite = InventorySlots[SelectedSlot].sprite;
			HotbarItems[SelectorCount] = objectId[SelectedSlot];
		}
		if(Input.GetKeyDown(KeyCode.E) && CamPlayer.SelectedConsole == null && string.IsNullOrEmpty(MainCam.GetComponent<OldPlayer>().WTMode)) {
			IP = true;
		} else {
			IP = false;
		}
		if(Input.anyKeyDown && IsInventoryOpen && string.IsNullOrEmpty(MainCam.GetComponent<OldPlayer>().WTMode)) {
